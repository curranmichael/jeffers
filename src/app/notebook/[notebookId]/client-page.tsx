...
function NotebookWorkspace({ notebookId }: { notebookId: string }) {
  ...
  useEffect(() => {
    ...
    // Check if we're in a static export environment (no window.api)
    const isStaticExport = typeof window !== 'undefined' && !window.api;
    
    if (isHydrated || isStaticExport) {
      // Calculate how long hydration took
      const hydrationTime = Date.now() - loadStartTime;
      const minimumAnimationTime = isStaticExport ? 0 : 600; // No delay for static exports
      
      console.log(`[NotebookWorkspace] Hydration completed in ${hydrationTime}ms ${isStaticExport ? '(static export)' : ''}`);
  ...
  // Guard: Ensure store is hydrated or we're in static export mode.
  const isStaticExport = typeof window !== 'undefined' && !window.api;
  if (!isHydrated && !isStaticExport) {
...
export function NotebookClientPage({ notebookId }: { notebookId: string }) {
  // Render the child component directly with the provided notebookId
